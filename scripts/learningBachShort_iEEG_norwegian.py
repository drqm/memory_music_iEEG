...
filename = logdir + '/' + subID[0] + '_learning_bach_iEEG_custom_norwegian.csv'
...
filename2 = logdir + '/' + subID[0] + '_learning_bach_iEEG_default_norwegian.log'
...
pd = visual.TextStim(win,text = 'Du skal nå lytte til et musikkstykke, som blir repetert to ganger.'
                                '\n\n Prøve å huske så mye av stykket som mulig.\n\n'
                                'Du skal senere utføre en hukommelsestest basert på denne musikken.'
                                '\n\n Trykk mellomromstasten for å begynne.',
                                color = color_txt, height = 0.1)
...
instrrec = visual.TextStim(win,text = 'Gjenkjennelse-del.',color = color_txt)
...
for wavve in range(len(wavefiles)):
    ...
    jes = 'Runde ' + str((wavve + 1)) + ' / ' + str(len(wavefiles))
...
#final message
playlear = visual.TextStim(win,text = 'Det var det. \n'
                                      'Tusen takk! \n\n '
                                      'Trykk mellomromstasten for å avslutte denne delen av forsøket.',
                            color = color_txt)
playlear.draw()
win.flip()
event.waitKeys(keyList = [SPAAAAACE])
